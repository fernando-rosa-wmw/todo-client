...
export class HttpTodoRepositoryService implements TodoRepository {
  ...
  async listAllTodo(): Promise<ListAllTodoOutput> {
    ...
    const response: HttpResponse = await this.httpClientService.post("http://localhost:8080/todo/listAll", headers, JSON.stringify(body));
